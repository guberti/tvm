...
def _write_out_sums(num_sums) -> Iterator[str]:
    ...
    packing plus a pipelined int32 store.
    ...
    for i in range(num_sums // 2):
        yield f"int packed_res_{i} = __builtin_arm_pkhtb(requant_{2 * i + 1}, requant_{2 * i});"

    for i in range(num_sums // 2):
        yield f"output[i] = packed_res_{i};"
